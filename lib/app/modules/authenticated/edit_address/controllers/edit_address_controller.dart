...
class EditAddressController extends GetxController {
  ...
  void onReady() async {
    ...
    addressLabelController.text = args['address_label'] ?? '';
    phoneController.text = args['phone_number'] ?? '';
    kodePosController.text = args['postal_code'] ?? '';
    locationMap.value = LocationModel(
      lat: args['lat'] ?? 0,
      long: args['long'] ?? 0,
      alamatLengkap: args['address_map'] ?? '',
    );
    addressController.text = args['address'];
    emailController.text = args['email'] ?? '';
    npwpController.text = args['npwp'] ?? '';
    final file = args['npwp_file'];
    npwpFile.value = (file is String && file.isNotEmpty) ? file : null;
    await getSubDistrict(args['sub_district']);
  }

  Future<void> getSubDistrict(int id) async {
    try {
      final response = await getUsecase.call(
        url: 'api/blueray/address/subdistricts/$id',
  ...
  Future<void> postAlamat() async {
    ...
    try {
      final response = await putUsecase.call(
        url: 'api/blueray/customer/address/${args['address_id']}',
        body: payload,
        converter: (json) => SuccessModel.fromJson(json),
      );
      response.fold(
